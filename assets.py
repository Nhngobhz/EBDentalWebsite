...
CSS_BUNDLES = {
    ...
    # the sign-in form.
    "store.css": [
        # Both generated by scripts/vendor_assets.py, and first in every bundle so
        # the @font-face rules are parsed before anything that sets font-family.
        # They used to be two render-blocking <link> tags to fonts.googleapis.com and
        # cdnjs - see that script for why a LAN-only site should not be waiting on
        # the public internet before it can paint.
        "fonts.css",
        "icons.css",
        "base.css",
        "layout.css",
